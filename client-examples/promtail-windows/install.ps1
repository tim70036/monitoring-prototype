...
.\nssm.exe install promtail-service $PROMTAIL_EXECUTABLE --config.file="promtail.yml" --config.expand-env=true
.\nssm.exe set promtail-service Application $PROMTAIL_EXECUTABLE
.\nssm.exe set promtail-service AppDirectory $PSScriptRoot
.\nssm.exe set promtail-service AppParameters --config.file="promtail.yml" --config.expand-env=true
